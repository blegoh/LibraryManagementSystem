                                ...
                                <div class="art-post-inner art-article">
                                    ...
                                    <div class="art-postcontent">
                                        <div class="form">
                                            ...
                                            <form action="<?=base_url('index.php/Collection/edit/'.$book->get_id())?>" method="post" id="form-koleksi" enctype="multipart/form-data">
                                                <label>Judul</label>
                                                <input type="text" name="judul" id="judul" value="<?=$book->get_title()?>" />
                                                <br />
                                                <label>Kategori</label>
                                                <select name="kategori" id="kategori">
                                                    <option value=""></option>
                                                    <?php foreach ($categories as $category) {?>
                                                        <option value="<?=$category->get_id()?>" <?php echo ($category->get_id() == $book->get_category()->get_id()) ? "selected":"" ?>>
                                                ...
                                                <select name="penerbit" id="penerbit">
                                                    ...
                                                    <?php foreach ($publishers as  $publisher) { ?>
                                                        <option value="<?=$publisher->get_id()?>" <?php echo ($publisher->get_id() == $book->get_publisher()->get_id()) ? "selected":"" ?>>
                                                            <?=$publisher->get_name()?></option>
                                                    <?php } ?>
                                                </select>
                                                <br />
                                                <label>Pengarang</label>
                                                <input type="text" name="pengarang" id="pengarang" value="<?=$book->get_author()?>" /><br />
                                                <label>Jumlah Halaman</label>
                                                <input type="text" name="halaman" id="halaman" value="<?=$book->get_num_pages()?>" /><br />
                                                ...
                                                <select name="thn" id="thn">
                                                    ...
                                                    for($i=1900;$i<=$tahun;$i++){ ?>
                                                        <option <?php echo ($i == $book->get_year()) ? "selected":"" ?>><?=$i?></option>
                                                    <?php } ?>
                                                </select>
                                                <br />
                                                <label>Deskripsi</label>
                                                <textarea rows="7" cols="30" id="deskripsi" name="deskripsi"><?=$book->get_detail()?></textarea><br />
                                                <label>Foto</label><input type="file" name="foto" /><br />
                                                <label>Harga</label>
                                                <input type="text" id="harga" name="harga" value="<?=$book->get_price()?>" />
                                                <br />
                                                <label>Jumlah Eksemplar</label>
                                                <input type="text" id="eksemplar" name="eksemplar" value="<?=$book->get_stock()?>" /><br />
